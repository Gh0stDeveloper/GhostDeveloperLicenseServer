...
install_dependencies(){
  ...
  for command in curl jq openssl tar sha256sum date flock; do command -v "$command" >/dev/null 2>&1 || missing=1; done
  ((missing == 0)) && return 0
  command -v apt-get >/dev/null 2>&1 || fail "Faltan dependencias y apt-get no está disponible."
  export DEBIAN_FRONTEND=noninteractive
  apt-get update -y
  apt-get install -y --no-install-recommends curl jq openssl ca-certificates tar coreutils util-linux
}
...
persist_authorization_state(){
  ...
  local subject="$5" version="$6" action="$7" public_key="$8"
  local work
...
main(){
  ...
  curl -fsSL --retry 3 --connect-timeout 10 --max-time 300 "$download_url" -o "$archive" || fail "No se pudo descargar el paquete privado autorizado."
